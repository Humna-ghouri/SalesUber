...
const Home = () => {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden flex items-center">
      ...
      <div className="relative z-10 text-white w-full max-w-[900px] px-6 md:px-16 lg:px-32 xl:px-48">
  <motion.h1
    className="text-5xl md:text-5xl font-black uppercase tracking-wide leading-tight"
  ...
  >
    {"Welcome to".split("").map((letter, index) => (
      <motion.span
        key={index}
        initial={{ opacity: 0, x: 50 }} // Right to left animation for letters
        animate={{ opacity: 1, x: 0 }}
    ...
    <span className="flex">
      <motion.span
        initial={{ opacity: 0, x: 100 }} // Right to left animation for "SalesUber"
        animate={{ opacity: 1, x: 0 }}
      ...
      <motion.span
        initial={{ opacity: 0, x: 100 }} // Right to left animation for "LLC"
        animate={{ opacity: 1, x: 0 }}
        transition={{ delay: 0.8, duration: 3 }} // Increased delay for the second word
      >
        LLC
      </motion.span>
    </span>
